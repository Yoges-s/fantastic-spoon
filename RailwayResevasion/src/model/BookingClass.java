...
public class BookingClass {
	...
	public void Book(String name, int age, char start, char end, String preference) {
		...
		if (t == null) {
			...
			if (t.getTotalTickets() > 0) {
				if (availConfirm > 0) {
					if (ticketCount == 0) {
						System.out.println("Ticket Not Available for Your " + "Preference..please try other");
						m.Booking();
					} else {
						if (age > 5) {
							if (preference.equalsIgnoreCase("lower")) {
								if (age > 60) {
									...
									passtic.put(++m.cid, (new Ticket(name, age, preference, start, end, t, m.cid)));

									System.out.println("Lower Berth Ticket Booked..Customer id-" + m.cid);
								} else if (age <= 5) {
									System.out.println(
								...
								passtic.put(++m.cid, (new Ticket(name, age, preference, start, end, t, m.cid)));
								System.out.println("Middle Berth Ticket Booked..CustomerId-" + m.cid);
							} else if (preference.equalsIgnoreCase("upper")) {
								t.setUpper(t.getUpper() - 1);
								t.setTotalTickets(t.getTotalTickets() - 1);
								t.setTupper(t.getTupper() + 1);
								passtic.put(++m.cid, (new Ticket(name, age, preference, start, end, t, m.cid)));
								System.out.println("Upper Berth Ticket Booked..Customer id-" + m.cid);
							} else if (preference.equalsIgnoreCase("sideUpper")) {
								t.setSideupper(t.getSideupper() - 1);
								t.setTotalTickets(t.getTotalTickets() - 1);
								t.setTsideupper(t.getTsideupper() + 1);
								passtic.put(++m.cid, (new Ticket(name, age, preference, start, end, t, m.cid)));
								System.out.println("Side Upper Ticket Booked..CustomerId-" + m.cid);
							}
						}
					...
					if (age > 5) {
						if (t.getRacTicket() > 0) {
							...
							if (t.getSideLower() > 0) {
								...
								passtic.put(++m.cid, (new Ticket(name, age, preference, start, end, t, m.cid)));
								System.out.println("Rac Side Lower is Booked-" + m.cid);
							} else {
								t.setTotalTickets(t.getTotalTickets() - 1);
								t.setRacTicket(t.getRacTicket() - 1);
								t.setTrac(t.getTrac() + 1);
								ractic.add((new Ticket(name, age, preference, start, end, t, ++m.cid)));
								passtic.put(m.cid, (new Ticket(name, age, preference, start, end, t, m.cid)));
								System.out.println("Rac is Booked Seating Details will send Later-" + m.cid);
							}
						} else {
							if (age > 5) {
								if (t.getWaitTicket() > 0) {
									...
									waittic.add((new Ticket(name, age, preference, start, end, t, ++m.cid)));
									passtic.put(m.cid, (new Ticket(name, age, preference, start, end, t, m.cid)));
									System.out
									.println("Waiting List is Booked Seating Details will send Later-" + m.cid);
								} else {
			...
			if (cid2 == map.getKey()) {
				if (passtic.containsKey(cid2)) {
					...
					if (s.equalsIgnoreCase("upper")) {
						...
						m2.setRacTicket(m2.getRacTicket()+1);
					}
				...
				System.out.println("cid-" + cid2 + "Not Available");
	...
	private void Rac(Ticket m1, Train m2) {
		Ticket rac = ractic.get(0);
		int c = rac.getCid();
		if (m2.getLower() > 0) {
			rac.setPreference("lower");
			m2.setLower(m2.getLower() - 1);
			System.out.println("Rac to Confirm Lower Berth Ticket Cid-"+c);
		} else if (m2.getMidle() > 0) {
			rac.setPreference("middle");
			m2.setMidle((m2.getMidle() - 1));
			System.out.println("Rac to Confirm Middle Berth Ticket Cid-"+c);
		} else if (m2.getUpper() > 0) {
			rac.setPreference("upper");
			m2.setUpper((m2.getUpper() - 1));
			System.out.println("Rac to Confirm Upper Berth Ticket Cid-"+c);
		} else if (m2.getSideLower() > 0) {
			rac.setPreference("sidelower");
			m2.setSideLower(m2.getSideLower() - 1);
			System.out.println("Rac to Confirm Side Lower Berth Ticket Cid-"+c);
		} else if (m2.getSideLower() > 0) {
			rac.setPreference("sideupper");
			m2.setSideupper(m2.getSideupper() - 1);
			System.out.println("Rac to Confirm Side Upper Berth Ticket Cid-"+c);
		}
		passtic.put(c, rac);
		ractic.remove(0);
		m2.setRacTicket(m2.getRacTicket() + 1);
		if (!waittic.isEmpty()) {
			Ticket t =waittic.get(0);
			System.out.println("Waiting List to Rac Moved "+t.getCid());
			ractic.add(waittic.get(0));
			waittic.remove(0);
